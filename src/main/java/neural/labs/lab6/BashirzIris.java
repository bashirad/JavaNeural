...
public class BashirzIris {
    ...
    static double[][] normalize(double[][] src) {
        ...
        IntStream.range(0,rowCount).forEach(rowno -> {
            IntStream.range(0, colCount).forEach(colno -> {
                ...
                normalizers.put((int)(mop.transpose(src)[colno][rowno]), norm);
            });
        });
        /**
    ...
    static final Equilateral eq =
            new Equilateral(IrisHelper.cat2Species.size(),
                    NORMALIZED_HI,
                    NORMALIZED_LO);
    /**
     *  Encode
    ...
    static double[][] encode(double[][] src) {
        ...
        IntStream.range(0, rowCount).forEach(rowno -> {
            IntStream.range(0, 1).forEach(colno -> {
                dest[rowno] = eq.encode((int)src[rowno][0]);
            });
        });
        return dest;
    }
    static double [][] decode(double [][] src) {
        int rowCount = src.length;
        IMop mop = new Mop();
        ...
        IntStream.range(0, rowCount).forEach(rowno -> {
            IntStream.range(0, 1).forEach(colno -> {
                dest[rowno][0] = eq.decode(src[rowno]);
            });
        });
...
//        final BasicTraining training = new Backpropagation(network, trainingSet,LEARNING_RATE,LEARNING_MOMENTUM);
        ...
        /**
         * Trying to output     t1      t2      y1      y2      using the network related methods
         */
        EncogHelper.describe(network);
        IMop mop = new Mop();
        mop.reportIdeals(trainingSet, network);
