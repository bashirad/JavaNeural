...
public class Dice4Test {
    ...
    @Test
    public void test() {
        final double[][] dice4 = mop.dice(TEST_MATRIX,1,4);

        int numRows = dice4.length;
        assert(numRows == EXPECTED_MATRIX.length);

        int numCols = dice4[0].length;
        assert(numCols == EXPECTED_MATRIX[0].length);

        IntStream.range(0,numRows).forEach( rowno -> {
            IntStream.range(0,numCols).forEach(colno -> {
                assert(dice4[rowno][colno] == EXPECTED_MATRIX[rowno][colno]);
            });
        });

        mop.print(this.getClass().getName()+" dice4",dice4);
    }
}
