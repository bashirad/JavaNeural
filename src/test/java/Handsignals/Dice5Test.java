...
public class Dice5Test {
    ...
    @Test
    public void test() {
        final double[][] dice5 = mop.dice(TEST_MATRIX,4,5);

        int numRows = dice5.length;
        assert(numRows == EXPECTED_MATRIX.length);

        int numCols = dice5[0].length;
        assert(numCols == EXPECTED_MATRIX[0].length);

        IntStream.range(0,numRows).forEach( rowno -> {
            IntStream.range(0,numCols).forEach(colno -> {
                assert(dice5[rowno][colno] == EXPECTED_MATRIX[rowno][colno]);
            });
        });

        mop.print(this.getClass().getName()+" dice5",dice5);
    }
}