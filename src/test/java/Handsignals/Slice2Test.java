...
public class Slice2Test {
    ...
    @Test
    public void test() {
        final double[][] slice2 = mop.slice(TEST_MATRIX,1,3);

        int numRows = slice2.length;
        assert(numRows == EXPECTED_MATRIX.length);

        int numCols = slice2[0].length;
        assert(numCols == EXPECTED_MATRIX[0].length);

        IntStream.range(0,numRows).forEach( rowno -> {
            IntStream.range(0,numCols).forEach(colno -> {
                assert(slice2[rowno][colno] == EXPECTED_MATRIX[rowno][colno]);
            });
        });

        mop.print(this.getClass().getName()+" slice2",slice2);
    }
}
