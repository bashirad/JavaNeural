...
public class Slice4Test {
    ...
    @Test
    public void test() {
        final double[][] slice4 = mop.slice(TEST_MATRIX,1,4);

        int numRows = slice4.length;
        assert(numRows == EXPECTED_MATRIX.length);

        int numCols = slice4[0].length;
        assert(numCols == EXPECTED_MATRIX[0].length);

        IntStream.range(0,numRows).forEach(rowno -> {
            IntStream.range(0,numCols).forEach(colno -> {
                assert(slice4[rowno][colno] == EXPECTED_MATRIX[rowno][colno]);
            });
        });

        mop.print(this.getClass().getName()+" slice4",slice4);
    }
}