...
public class Slice6Test {
    ...
    @Test
    public void test() {
        final double[][] slice6 = mop.slice(TEST_MATRIX,2,3);

        int numRows = slice6.length;
        assert(numRows == EXPECTED_MATRIX.length);

        int numCols = slice6[0].length;
        assert(numCols == EXPECTED_MATRIX[0].length);

        IntStream.range(0,numRows).forEach(rowno -> {
            IntStream.range(0,numCols).forEach(colno -> {
                assert(slice6[rowno][colno] == EXPECTED_MATRIX[rowno][colno]);
            });
        });

        mop.print(this.getClass().getName()+" slice6",slice6);
    }
}