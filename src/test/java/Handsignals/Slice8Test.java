...
public class Slice8Test {
    ...
    @Test
    public void test() {
        final double[][] slice8 = mop.slice(TEST_MATRIX,6,9);

        int numRows = slice8.length;
        assert(numRows == EXPECTED_MATRIX.length);

        int numCols = slice8[0].length;
        assert(numCols == EXPECTED_MATRIX[0].length);

        IntStream.range(0,numRows).forEach(rowno -> {
            IntStream.range(0,numCols).forEach(colno -> {
                assert(slice8[rowno][colno] == EXPECTED_MATRIX[rowno][colno]);
            });
        });

        mop.print(this.getClass().getName()+" slice8",slice8);
    }
}