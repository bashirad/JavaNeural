...
public class Transpose5Test {
    ...
    @Test
    public void test() {
        final double[][] transpose5 = mop.transpose(TEST_MATRIX);

        int numRows = transpose5.length;
        assert(numRows == EXPECTED_MATRIX.length);

        int numCols = transpose5[0].length;
        assert(numCols == EXPECTED_MATRIX[0].length);

        IntStream.range(0,numRows).forEach(rowno -> {
            IntStream.range(0,numCols).forEach(colno -> {
                assert(transpose5[rowno][colno] == EXPECTED_MATRIX[rowno][colno]);
            });
        });

        mop.print(this.getClass().getName()+" transpose5",transpose5);
    }
}