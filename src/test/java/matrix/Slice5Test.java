...
public class Slice5Test {
    ...
    @Test
    public void test() {
        final double[][] slice5 = mop.slice(TEST_MATRIX,4,5);

        int numRows = slice5.length;
        assert(numRows == EXPECTED_MATRIX.length);

        int numCols = slice5[0].length;
        assert(numCols == EXPECTED_MATRIX[0].length);

        IntStream.range(0,numRows).forEach(rowno -> {
            IntStream.range(0,numCols).forEach(colno -> {
                assert(slice5[rowno][colno] == EXPECTED_MATRIX[rowno][colno]);
            });
        });

        mop.print(this.getClass().getName()+" slice5",slice5);
    }
}