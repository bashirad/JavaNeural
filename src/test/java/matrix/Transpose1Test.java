...
public class Transpose1Test {
    ...
    @Test
    public void test() {
        final double[][] transpose1 = mop.transpose(TEST_MATRIX);

        int numRows = transpose1.length;
        assert(numRows == EXPECTED_MATRIX.length);

        int numCols = transpose1[0].length;
        assert(numCols == EXPECTED_MATRIX[0].length);

        IntStream.range(0,numRows).forEach( rowno -> {
            IntStream.range(0,numCols).forEach(colno -> {
                assert(transpose1[rowno][colno] == EXPECTED_MATRIX[rowno][colno]);
            });
        });

        mop.print(this.getClass().getName()+" transpose1",transpose1);
    }
}
